...
class AuthRepositoryImpl implements AuthRepository {
  ...
  @override
  Future<Result<Unit>> singIn() async {
    final result = await _googleSigninService.signIn();
    switch (result) {
      case Success<String>(:final value):
        await _localStorageService.saveIdToken(value);
        // Vamos fazer o loging no back
        return successOfUnit();
      case Failure<String>(:final error):
        log(
          'Erro ao realizar login',
          name: 'Auth Respository',
          error: error,
        );
        return Failure(
          DataException(message: 'Erro ao realizer login com a google'),
        );
    }
  }
  ...
  Future<Result<Unit>> singOut() async {
    ...
    switch (result) {
      case Success<Unit>():
        final removeResult = await _localStorageService.removerIdToken();
        switch (removeResult) {
          case Success<Unit>():
            return successOfUnit();
          case Failure<Unit>(:final error):
            log('Erro realizar logout ID token');
